...
def findq(bot, trigger):
    ...
    options.host = bot.config.quote.host
    options.owner = bot.config.core.owner
    options.webbase = bot.config.quote.webbase
    ...
    if data is None or data == '':
        ...
        if output == False:
            ...
            if ( quote_count == 1 ):
                bot.say("%s: Found %s result:" % (options.nick, quote_count) )
            elif quote_count in range(2, 5):
                bot.say("%s: Found these %s results:" % (options.nick, quote_count) )
            else:
                bot.say("%s: Found %s results. Sorting 5 randomly:" % (options.nick, quote_count) )
            
            for row in itertools.islice(output, 5):
                bot.say("[%s]: %s" % (row[0], row[3]) )
                time.sleep(1)
            
            if (quote_count > 5):
                remaining = quote_count - 5
                url_data = urllib.quote_plus(data)
                bot.say("Plus %s more matches: %s/?filter=%s" % (remaining, options.webbase, url_data) )
...
class MySQLQuoteDataProvider(QuoteDataProvider):
    ...
    def search(self, data):
        #sql = "SELECT * FROM quotes WHERE quote LIKE %s ORDER BY RAND() LIMIT 5"
        sql = "SELECT * FROM quotes WHERE quote LIKE %s ORDER BY RAND()"
        rows_count = self.dbcursor.execute(sql, ("%" + data + "%",))
        if rows_count > 0:
